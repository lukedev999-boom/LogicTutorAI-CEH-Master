...
> 📁 **圖片存放位置**：圖片檔一律放在 `images/`（與 `index.html` 同層）。
>
> 題庫中的路徑寫法不拘 —— `../images/q240.png`、`images/q240.png`，甚至只寫
> `q240.png`，系統都會正規化為指向 `images/`。若要分子目錄管理，寫成
> `../images/2024/q1.png`，`images/` 之後的結構會被保留。
>
> 之所以不能沿用「相對於題庫 md 檔」的路徑：瀏覽器解析 `<img src>` 的基準是
> **目前頁面的 URL**，不是 md 檔的位置；而且用「載入題庫」按鈕選檔時，瀏覽器
> 基於安全考量不會提供 md 檔的磁碟路徑。因此圖片只能放在網站目錄底下。
>
> 圖片找不到時，畫面會就地顯示提示框說明缺少哪個檔案，不會留下破圖。
>
> ⚠️ 依專案設定，`images/` 內的圖片**不進版控**，部署時請記得另外放置。
...
- **圖片路徑正規化**：把題庫中各種寫法的圖片路徑統一指向 `images/`
